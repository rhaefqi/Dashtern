 ...
 <div class="flex h-screen">
     ...
     <!-- Sidebar -->
     <aside x-data="{ open: true }" class="bg-white shadow-md"
           :class="open ? 'w-64' : 'w-20'">
        <!-- Logo dan Nama -->
        <div class="flex items-center justify-center py-6">
            <div class="w-10 h-10">
                <img src="/image/logo.png" alt="Logo" @click="open = !open" class="w-full h-full object-contain" />
            </div>
            <span x-show="open" class="ml-2 text-2xl font-bold text-[#145A5A]">Dashtern</span>

            <!-- Toggle Sidebar Button -->
            <div x-show="open" class="flex justify-end pl-8 my-4 pt-">
         ...
         <nav class="w-full px-4 space-y-4">
                <a href="{{ url('/beranda') }}"
                    class="flex items-center space-x-3 px-4 py-3 trainsition rounded-lg {{ request()->is('beranda') || request()->is('/*') ? 'bg-[#145A5A] text-white shadow' : 'text-black hover:bg-gray-100' }}">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 2L2 8v10a1 1 0 001 1h5V12h4v7h5a1 1 0 001-1V8l-8-6z" />
                    </svg>
                    <span x-show="open">Beranda</span>
                </a>
                 <a href="{{ $mahasiswa_login && $mahasiswa_login->kode_kelas ? '/kelas' : '/gabung' }}"
                    class="flex items-center space-x-3 px-4 py-3 trainsition rounded-lg
                            {{ request()->is('kelas*') || request()->is('gabung*') || request()->is('tugas*') || request()->is('form*') ? 'bg-[#145A5A] text-white shadow' : 'text-black hover:bg-gray-100' }}">
                 ...
                 <a href="{{ route('progres') }}"
                     ...
                     <span x-show="open">Kinerja Magang</span>
                 </a>
                 ...
                 <a href="/tentang"
                     ...
                     <span x-show="open">Tentang Kami</span>
                 </a>
                 <a href="/profil"
                 ...
                 <div class="pt-10">
                     <form method="POST" action="{{ route('logout') }}">
                         @csrf
                         <button type="submit"
                             class="w-full text-left flex items-center space-x-3 px-4 py-3 rounded-lg shadow' : 'text-black hover:bg-gray-100' }}">
